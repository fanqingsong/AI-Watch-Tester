...
class SimpleSupervisorAgent:
    ...
    async def initialize(self):
        ...
        self.llm = ChatAnthropic(
            model=self.config.ai_model.split(":")[-1],  # 提取模型名
            temperature=0.7,
            timeout=60.0
        )
    ...
    async def _execute_test_flow(self, mode: str) -> TestResult:
        ...
        for i, step in enumerate(test_plan.get("steps", []), 1):
            ...
            # 检查是否需要用户确认
            if mode == "conservative" or (mode == "interactive" and step.get("needs_confirmation")):
                user_input = input(f"  执行此步骤? (y/n/q): ").strip().lower()
                if user_input == 'q':
                    print("  ⏹️  用户中止测试")
                    break
                elif user_input == 'n':
    ...
    async def _attempt_fix(self, step: Dict[str, Any], error_result: Dict[str, Any]) -> Dict[str, Any]:
        """尝试修复失败的步骤"""
        # 简化的修复逻辑
