...
class AgentConfig(BaseModel):
    ...
    ai_model: str = Field(
        ...
        description="主要使用的 AI 模型"
    )
    backup_model: Optional[str] = Field(
        default=None,
        description="备用 AI 模型"
    )

    # 代理行为配置
    default_mode: AgentMode = Field(
        default=AgentMode.INTERACTIVE,
        description="默认运行模式"
    )
    strategy_level: StrategyLevel = Field(
        default=StrategyLevel.BALANCED,
        description="策略级别"
    )
    ...
    max_exploration_depth: int = Field(
        default=3,
        description="最大探索深度（页面跳转层级）"
    )
    exploration_timeout: int = Field(
        default=30000,
        description="单页面探索超时时间（毫秒）"
    )

    # 测试配置
    test_execution_timeout: int = Field(
        default=60000,
        description="测试执行超时时间（毫秒）"
    )
    ...
    # 记忆和学习配置
    enable_exploration_memory: bool = Field(
        default=True,
        description="是否启用探索记忆"
    )
    enable_user_learning: bool = Field(
        default=True,
        description="是否启用用户偏好学习"
    )
    ...
    enable_sandbox: bool = Field(
        default=True,
        description="是否启用沙盒隔离"
    )
    sandbox_timeout: int = Field(
        default=120000,
        description="沙盒执行超时时间（毫秒）"
    )

    # 用户交互配置
    enable_human_feedback: bool = Field(
        default=True,
        description="是否启用人机交互反馈"
    )
    feedback_frequency: str = Field(
        default="on_failure",
        description="反馈频率：always | on_failure | on_milestone"
    )
